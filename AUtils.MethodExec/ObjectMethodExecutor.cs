...
{
    private readonly object?[]? mParameterDefaultValues;
    private readonly MethodExecutorAsync? mExecutorAsync;
    private readonly MethodExecutor? mExecutor;
    ...
    private ObjectMethodExecutor(MethodInfo methodInfo, TypeInfo targetTypeInfo, object?[]? parameterDefaultValues)
    {
        ...
        mExecutor = GetExecutor(methodInfo, targetTypeInfo);

        if (IsMethodAsync)
        {
            mExecutorAsync = GetExecutorAsync(methodInfo, targetTypeInfo, coercedAwaitableInfo);
        }

        mParameterDefaultValues = parameterDefaultValues;
    }
    ...
    {
        Debug.Assert(mExecutor != null, "Sync execution is not supported.");
        return mExecutor(target, parameters);
    }
    ...
    {
        Debug.Assert(mExecutorAsync != null, "Async execution is not supported.");
        return mExecutorAsync(target, parameters);
    }

    public object? GetDefaultValueForParameter(int index)
    {
        if (mParameterDefaultValues == null)
        {
        ...
        return mParameterDefaultValues[index];
    }
