...
internal struct PoolingAsyncStateMachine<TState, TResult> : IAsyncStateMachine<TState>
    where TState : struct
{
    private readonly Transition<TState, PoolingAsyncStateMachine<TState, TResult>> transition;

    /// <summary>
    /// Represents internal state.
    /// </summary>
    public TState State;
    private PoolingAsyncValueTaskMethodBuilder<TResult?> builder;
    private ExceptionDispatchInfo? exception;
    private bool suspended;
    private uint guardedRegionsCounter;    // number of entries into try-clause
    private TResult? result;

    private PoolingAsyncStateMachine(Transition<TState, PoolingAsyncStateMachine<TState, TResult>> transition, TState state)
    {
        builder = PoolingAsyncValueTaskMethodBuilder<TResult?>.Create();
    ...
    public static ValueTask<TResult?> Start(Transition<TState, PoolingAsyncStateMachine<TState, TResult>> transition, TState initialState = default)
        => new PoolingAsyncStateMachine<TState, TResult>(transition, initialState).Start();
