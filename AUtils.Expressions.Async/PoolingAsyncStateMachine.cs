...
internal struct PoolingAsyncStateMachine<TState> : IAsyncStateMachine<TState>
    where TState : struct
{
    private readonly Transition<TState, PoolingAsyncStateMachine<TState>> transition;

    /// <summary>
    /// Runtime state associated with this state machine.
    /// </summary>
    public TState State;
    private PoolingAsyncValueTaskMethodBuilder builder;
    private ExceptionDispatchInfo? exception;
    private bool suspended;
    private uint guardedRegionsCounter;    // number of entries into try-clause

    private PoolingAsyncStateMachine(Transition<TState, PoolingAsyncStateMachine<TState>> transition, TState state)
    {
        builder = PoolingAsyncValueTaskMethodBuilder.Create();
    ...
    public static ValueTask Start(Transition<TState, PoolingAsyncStateMachine<TState>> transition, TState initialState = default)
        => new PoolingAsyncStateMachine<TState>(transition, initialState).Start();
